...
python3 "$AICC_CHECKER" extract "$WORK_DIR/extracted/data.sql" "$AICC_EXPECTED_COUNTS"
python3 "$AICC_CHECKER" verify "$AICC_EXPECTED_COUNTS"
docker exec --env PGPASSWORD="$LOCAL_DB_PASSWORD" "$ISOLATED_DB_CONTAINER" \
  psql -h /var/run/postgresql -U supabase_admin -d postgres -XAtq -F $'\t' \
...
python3 "$AICC_CHECKER" verify "$AICC_ACTUAL_COUNTS"
python3 "$AICC_CHECKER" compare "$AICC_EXPECTED_COUNTS" "$AICC_ACTUAL_COUNTS"
...
# session_replication_role=replica bypasses FK enforcement during restore. Check
...
echo 'L1B B-2 role graph, AICC counts, effective privileges, complete FK, function ACL and private-object verification: PASS'
