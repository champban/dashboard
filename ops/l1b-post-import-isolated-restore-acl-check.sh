...
), expected_kinds(kind) as (
  values ('RELMETA'),('RELACL'),('COLACL'),('FUNMETA'),('FUNACL')
), expected_keys(key) as (
  select scope from expected_scopes
  union all
  select scope||'_'||kind from expected_scopes cross join expected_kinds
), diagnostic_parts as (
  select scope as key,part from parts
  union all
  select scope||'_'||split_part(part,'|',1),part from parts
)
select expected_keys.key,count(diagnostic_parts.part),
       md5(coalesce(string_agg(diagnostic_parts.part,E'\n'
                               order by diagnostic_parts.part),''))
from expected_keys
left join diagnostic_parts on diagnostic_parts.key=expected_keys.key
group by expected_keys.key
order by expected_keys.key;
SQL
