...
# Frozen durable user-authentication state required to restore existing
# OAuth identities, sessions, refresh tokens, MFA state and WebAuthn factors.
# Transient OAuth flow/challenge ledgers and provider bootstrap metadata are
# intentionally regenerated by Supabase rather than certified as user state.
...
TARGET_RELATIONS = AUTH_RECOVERY_RELATIONS + PUBLIC_RECOVERY_RELATIONS
...
def check_contract(workflow: Path, restore_script: Path) -> None:
    ...
    required_workflow = (
        ...
        "EXPECTED_SOURCE_SHA: 25508b077a7ee81c1b6b6f42da2dbd704a7f247f",
    ...
    cleanup_marker = "L1B_B2_UNCONDITIONAL_CLEANUP_COMPLETE"
    if wf.count(cleanup_marker) != 1:
        raise ValueError("workflow must contain exactly one unconditional cleanup marker")
    if not wf.index(cleanup_marker) < wf.index("steps.restore.outcome") < wf.index(final_marker):
        raise ValueError("cleanup proof, restore outcome and final recovery PASS are not fail-closed ordered")
    forbidden_script = (
        r"\.supabase\.co",
        r"pooler\.supabase",
        r"https?://",
