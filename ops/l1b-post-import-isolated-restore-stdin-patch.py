...
STAGES=("exact-count","postcheck","catalog","index","raw-acl","default-acl","all-function-acl","effective-privilege","role-graph","aicc-count","auth-fk-fingerprint","fk-fingerprint","fk-row-validation","private-storage")
...
def patch_base_acl(payload):
    ...
    new=("l1b_b2_assert_raw_acl AICC \"$EXPECTED_AICC_ACL_PARTS\" \"$EXPECTED_AICC_ACL_FINGERPRINT\" \"$ACL_FINGERPRINTS\" \"$WORK_DIR/fingerprints.tsv\"\n" "l1b_b2_assert_raw_acl L0B \"$EXPECTED_L0B_ACL_PARTS\" \"$EXPECTED_L0B_ACL_FINGERPRINT\" \"$ACL_FINGERPRINTS\" \"$WORK_DIR/fingerprints.tsv\"\n" "l1b_b2_assert_raw_acl LINE \"$EXPECTED_LINE_ACL_PARTS\" \"$EXPECTED_LINE_ACL_FINGERPRINT\" \"$ACL_FINGERPRINTS\" \"$WORK_DIR/fingerprints.tsv\"\n\n" "l1b_b2_set_stage 'default-acl'\n\n")
...
def patch_review(payload):
    ...
    payload=replace_once(payload,'# Freeze the complete durable Auth FK inventory before validating any rows.\ndocker exec --interactive',"# Freeze the complete durable Auth FK inventory before validating any rows.\nl1b_b2_set_stage 'auth-fk-fingerprint'\ndocker exec --interactive",'review auth fk fingerprint stage')
    payload=replace_once(payload,"grep -Fxq \"$EXPECTED_AUTH_FK_PARTS\"$'\\t'\"$EXPECTED_AUTH_FK_FINGERPRINT\" \"$AUTH_FK_FINGERPRINT\"\n","l1b_b2_assert_pair auth-fk-fingerprint AUTH_FK \"$EXPECTED_AUTH_FK_PARTS\" \"$EXPECTED_AUTH_FK_FINGERPRINT\" \"$AUTH_FK_FINGERPRINT\"\n",'review auth fk fingerprint assertion')
