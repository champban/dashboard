...
WORK_DIR="$RUNNER_TEMP/dashboard-l1b-post-import-backup-$STAMP"
VERIFY_DIR="$RUNNER_TEMP/dashboard-l1b-post-import-verify-$STAMP"
PLAIN_ARCHIVE="$RUNNER_TEMP/dashboard-l1b-post-import-backup-$STAMP.tar.gz"
DECRYPTED_ARCHIVE="$RUNNER_TEMP/dashboard-l1b-post-import-backup-$STAMP.verify.tar.gz"
ENCRYPTED_ARCHIVE="$RUNNER_TEMP/dashboard-l1b-post-import-backup-$STAMP.tar.gz.gpg"
EXTERNAL_MANIFEST="$RUNNER_TEMP/dashboard-l1b-post-import-backup-$STAMP.manifest.txt"
PASSPHRASE_FILE="$RUNNER_TEMP/.dashboard-l1b-backup-passphrase-$STAMP"
...
backup_utc=$STAMP
purpose=l1b_fresh_post_import_recovery_gate
...
plaintext_retention=ephemeral_runner_only
EOF
...
{
  echo "## L1B fresh post-import encrypted Supabase backup"
  echo
  ...
  echo "- Production operation type: **logical read/export only**"
  echo
  echo "No connection URI, password, passphrase, SQL content, owner identifier, or private row data is written to this summary."
} >> "$GITHUB_STEP_SUMMARY"

echo "L1B fresh post-import encrypted logical backup verification: PASS"
