...
if ! docker exec --interactive --env PGPASSWORD="$LOCAL_DB_PASSWORD" "$ISOLATED_DB_CONTAINER" \
  psql -h /var/run/postgresql -U "$LOCAL_DB_ADMIN" -d postgres -Xq \
  --single-transaction --set=ON_ERROR_STOP=1 \
  >"$WORK_DIR/post-restore-check.log" 2>&1 <<'SQL'
do $$
declare
...
SQL
then
  echo "::error::Restored schema/RLS/ACL/function/reconciliation checks failed; raw diagnostics were withheld and deleted"
  exit 1
fi
