...
: "${EXPECTED_PACKET_A_ACL_MIGRATION_SHA256:?EXPECTED_PACKET_A_ACL_MIGRATION_SHA256 is required}"
...
PACKET_A_ACL_MIGRATION="$GITHUB_WORKSPACE/supabase/migrations/20260820083714_line_acl_default_privilege_hardening.sql"
if [[ "$EXPECTED_PACKET_A_ACL_MIGRATION_SHA256" != "$IMMUTABLE_PACKET_A_ACL_MIGRATION_SHA256" \
   || ! -f "$PACKET_A_ACL_MIGRATION" \
   || -L "$PACKET_A_ACL_MIGRATION" \
   || "$(sha256sum "$PACKET_A_ACL_MIGRATION" | awk '{print $1}')" != "$EXPECTED_PACKET_A_ACL_MIGRATION_SHA256" ]]; then
  echo "::error::Packet A ACL migration source differs from the reviewed immutable contract"
  exit 1
fi
...
# A Supabase bootstrap grants API roles broad creator defaults. Normalize only
# the pinned Packet A postgres defaults before schema creation, then restore the
# source schema as postgres so target-only ACLs cannot survive owner changes.
# The commands remain inside the roles/schema/data single transaction.
PHASE_NONCE=''
...
begin
  ...
  loop
    expected_privilege := case
      when privilege_record.role_name = 'authenticated'
        and privilege_record.table_name = 'mtp_line_accounts'
        and privilege_record.privilege_name = 'SELECT' then true
      when privilege_record.role_name = 'authenticated'
        and privilege_record.table_name = 'mtp_line_link_codes'
        and privilege_record.privilege_name in ('SELECT', 'INSERT', 'UPDATE') then true
      when privilege_record.role_name = 'authenticated'
        and privilege_record.table_name = 'mtp_line_mutations'
        and privilege_record.privilege_name = 'SELECT' then true
      when privilege_record.role_name = 'authenticated'
        and privilege_record.table_name = 'mtp_line_snapshots'
        and privilege_record.privilege_name in ('SELECT', 'INSERT', 'UPDATE') then true
      when privilege_record.role_name = 'service_role'
        and privilege_record.table_name = 'mtp_line_accounts'
        and privilege_record.privilege_name in ('SELECT', 'UPDATE') then true
      when privilege_record.role_name = 'service_role'
        and privilege_record.table_name = 'mtp_line_mutations'
        and privilege_record.privilege_name in ('SELECT', 'INSERT', 'UPDATE') then true
      when privilege_record.role_name = 'service_role'
        and privilege_record.table_name = 'mtp_line_events'
        and privilege_record.privilege_name in ('SELECT', 'UPDATE') then true
      when privilege_record.role_name = 'service_role'
        and privilege_record.table_name = 'mtp_line_snapshots'
        and privilege_record.privilege_name = 'SELECT' then true
    ...
    actual_privilege := pg_catalog.has_table_privilege(
      ...
      'public.' || privilege_record.table_name,
      privilege_record.privilege_name
    );
    if actual_privilege is distinct from expected_privilege then
      raise exception 'unexpected restored %.% % privilege for %',
        'public', privilege_record.table_name, privilege_record.privilege_name,
        privilege_record.role_name;
    end if;
    if pg_catalog.has_table_privilege(
      privilege_record.role_name,
      'public.' || privilege_record.table_name,
      privilege_record.privilege_name || ' WITH GRANT OPTION'
    ) then
      raise exception 'unexpected restored %.% % grant option for %',
        'public', privilege_record.table_name, privilege_record.privilege_name,
        privilege_record.role_name;
    end if;
  end loop;
end;
$packet_a_b2_table_acl$;

\warn :column_acl_marker
do $packet_a_b2_column_acl$
declare
...
begin
  if (
    select count(*)
     ...
     where attribute.attrelid = 'public.mtp_line_mutations'::regclass
    ...
    raise exception 'required LINE mutation completion columns are incomplete';
  end if;
  ...
  loop
    expected_privilege := case
      when column_record.role_name = 'authenticated'
        and column_record.table_name = 'mtp_line_accounts'
        and column_record.privilege_name = 'SELECT' then true
      when column_record.role_name = 'authenticated'
        and column_record.table_name = 'mtp_line_link_codes'
        and column_record.privilege_name in ('SELECT', 'INSERT', 'UPDATE') then true
      when column_record.role_name = 'authenticated'
        and column_record.table_name = 'mtp_line_mutations'
        and column_record.privilege_name = 'SELECT' then true
      when column_record.role_name = 'authenticated'
        and column_record.table_name = 'mtp_line_mutations'
        and column_record.privilege_name = 'UPDATE'
        and column_record.column_name in (
          'status', 'error_code', 'applied_at', 'updated_at'
        ) then true
      when column_record.role_name = 'authenticated'
        and column_record.table_name = 'mtp_line_snapshots'
        and column_record.privilege_name in ('SELECT', 'INSERT', 'UPDATE') then true
      when column_record.role_name = 'service_role'
        and column_record.table_name = 'mtp_line_accounts'
        and column_record.privilege_name in ('SELECT', 'UPDATE') then true
      when column_record.role_name = 'service_role'
        and column_record.table_name = 'mtp_line_events'
        and column_record.privilege_name in ('SELECT', 'UPDATE') then true
      when column_record.role_name = 'service_role'
        and column_record.table_name = 'mtp_line_mutations'
        and column_record.privilege_name in ('SELECT', 'INSERT', 'UPDATE') then true
    ...
    actual_privilege := pg_catalog.has_column_privilege(
      column_record.role_name,
      'public.' || column_record.table_name,
      column_record.column_name,
      column_record.privilege_name
    );
    if actual_privilege is distinct from expected_privilege then
      raise exception 'unexpected restored %.% column % % privilege for %',
        'public', column_record.table_name, column_record.column_name,
        column_record.privilege_name, column_record.role_name;
    end if;
    if pg_catalog.has_column_privilege(
      column_record.role_name,
      'public.' || column_record.table_name,
      column_record.column_name,
      column_record.privilege_name || ' WITH GRANT OPTION'
    ) then
      raise exception 'unexpected restored %.% column % % grant option for %',
        'public', column_record.table_name, column_record.column_name,
        column_record.privilege_name, column_record.role_name;
    end if;
  end loop;
end;
$packet_a_b2_column_acl$;

\warn :function_acl_marker
do $packet_a_b2_function_acl$
declare
...
begin
  ...
  loop
    actual_privilege := pg_catalog.has_function_privilege(
      function_record.role_name,
      function_record.signature,
      'EXECUTE'
    );
    if actual_privilege is distinct from (
      function_record.role_name = 'service_role'
    ) then
      raise exception 'unexpected restored EXECUTE on % for %',
        function_record.signature, function_record.role_name;
    end if;
    if pg_catalog.has_function_privilege(
      ...
      raise exception 'unexpected restored EXECUTE grant option on % for %',
        function_record.signature, function_record.role_name;
    end if;
  end loop;
end;
$packet_a_b2_function_acl$;

\warn :default_acl_marker
do $packet_a_b2_probe_precondition$
begin
  if pg_catalog.to_regclass('public.packet_a_b2_acl_probe') is not null
     or pg_catalog.to_regclass('public.packet_a_b2_acl_probe_seq') is not null
     or pg_catalog.to_regprocedure('public.packet_a_b2_acl_probe_fn()') is not null then
    raise exception 'Packet A ACL probe name already exists in the restored backup';
  end if;
end;
$packet_a_b2_probe_precondition$;

set role postgres;
create table public.packet_a_b2_acl_probe(id bigint);
create sequence public.packet_a_b2_acl_probe_seq;
create function public.packet_a_b2_acl_probe_fn()
returns integer language sql as $packet_a_b2_probe_fn$ select 1 $packet_a_b2_probe_fn$;
reset role;

do $packet_a_b2_default_acl$
declare
  privilege_record record;
begin
  if (
    select pg_catalog.pg_get_userbyid(relation.relowner)
     ...
     where relation.oid = 'public.packet_a_b2_acl_probe'::regclass
  ) is distinct from 'postgres'
     or (
       select pg_catalog.pg_get_userbyid(relation.relowner)
         from pg_catalog.pg_class as relation
        where relation.oid = 'public.packet_a_b2_acl_probe_seq'::regclass
     ) is distinct from 'postgres'
     or (
       select pg_catalog.pg_get_userbyid(function_entry.proowner)
         from pg_catalog.pg_proc as function_entry
        where function_entry.oid =
          'public.packet_a_b2_acl_probe_fn()'::regprocedure
     ) is distinct from 'postgres' then
    raise exception 'Packet A ACL probe is not postgres-owned';
  end if;
  ...
  loop
    if pg_catalog.has_table_privilege(
      privilege_record.role_name,
      'public.packet_a_b2_acl_probe',
      privilege_record.privilege_name
    ) then
      raise exception 'future postgres-owned table leaked % to %',
        privilege_record.privilege_name, privilege_record.role_name;
    end if;
  end loop;
  ...
  loop
    if pg_catalog.has_sequence_privilege(
      privilege_record.role_name,
      'public.packet_a_b2_acl_probe_seq',
      privilege_record.privilege_name
    ) then
      raise exception 'future postgres-owned sequence leaked % to %',
        privilege_record.privilege_name, privilege_record.role_name;
    end if;
  end loop;
  ...
  loop
    if pg_catalog.has_function_privilege(
      privilege_record.role_name,
      'public.packet_a_b2_acl_probe_fn()',
      'EXECUTE'
    ) then
      raise exception 'future postgres-owned function leaked EXECUTE to %',
        privilege_record.role_name;
    end if;
  end loop;
end;
$packet_a_b2_default_acl$;
...
then
  ...
  if post_restore_classification="$(
    python3 "$CHECKER" classify-post-restore "$POST_RESTORE_LOG" "$PHASE_NONCE" 2>>"$POST_RESTORE_LOG"
  )" && [[ "$post_restore_classification" =~ ^group=(baseline|table_acl|column_acl|function_acl|default_acl)[[:space:]]sqlstate=[0-9A-Z]{5}$ ]]; then
    echo "::error::Restored schema/RLS/ACL/function/reconciliation checks failed ($post_restore_classification); raw diagnostics were withheld and deleted"
  else
    echo "::error::Restored schema/RLS/ACL/function/reconciliation checks failed (group=unknown sqlstate=unknown); raw diagnostics were withheld and deleted"
  fi
  exit 1
fi
...

{
  ...
  echo "- Exact Packet A table/column/function ACL and postgres-owned default privileges: **PASS**"
