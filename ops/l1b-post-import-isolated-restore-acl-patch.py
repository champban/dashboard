...
import pathlib, sys
COUNT_CHECKER_BLOB='e4ba0307a3154ab00bff7bbd016bf5f08bb4771c'
...
'''
    ...
    old_fingerprint_select="select scope,count(*),md5(string_agg(part,E'\\n' order by part)) from parts where scope is not null group by scope order by scope;"
    new_fingerprint_select=(", expected_scopes(scope) as (values ('AICC'),('L0B'),('LINE'))"
        ", expected_kinds(kind) as (values ('REL'),('COL'),('POL'),('FUN'),('GRANT'))"
        ", expected_keys(key) as (select scope from expected_scopes union all select scope||'_'||kind from expected_scopes cross join expected_kinds)"
        ", diagnostic_parts as (select scope as key,part from parts where scope is not null union all select scope||'_'||split_part(part,'|',1),part from parts where scope is not null)"
        " select expected_keys.key,count(diagnostic_parts.part),md5(coalesce(string_agg(diagnostic_parts.part,E'\\n' order by diagnostic_parts.part),''))"
        " from expected_keys left join diagnostic_parts on diagnostic_parts.key=expected_keys.key"
        " group by expected_keys.key order by expected_keys.key;")
    payload=replace_once(payload,old_fingerprint_select,new_fingerprint_select,'privacy-safe-catalog-diagnostics')
    payload=replace_once(payload,'docker exec --interactive --env PGPASSWORD=\"$LOCAL_DB_PASSWORD\" \"$ISOLATED_DB_CONTAINER\" \\\n  psql -h /var/run/postgresql -U \"$LOCAL_DB_ADMIN\" -d postgres -XAtq -F $\'\\t\' --set=ON_ERROR_STOP=1 >\"$FINGERPRINTS\" 2>&1 <<\'SQL\'\n','if ! docker exec --interactive --env PGPASSWORD=\"$LOCAL_DB_PASSWORD\" \"$ISOLATED_DB_CONTAINER\" \\\n  psql -h /var/run/postgresql -U \"$LOCAL_DB_ADMIN\" -d postgres -XAtq -F $\'\\t\' --set=ON_ERROR_STOP=1 >\"$FINGERPRINTS\" 2>&1 <<\'SQL\'\n','catalog-query-guard')
    payload=replace_once(payload,'>\"$FINGERPRINTS\" 2>&1 <<\'SQL\'\nwith parts as (','>\"$FINGERPRINTS\" 2>&1 <<\'SQL\'\nset search_path to pg_catalog, public, extensions;\nwith parts as (','catalog-search-path')
    ...
    new_catalog=("SQL\nthen\n  printf '::error::L1B_B2_CATALOG_QUERY_FAILED stage=catalog\\n' >&2\n  exit 97\nfi\n\n" "l1b_b2_assert_triplet catalog AICC_REL \"$EXPECTED_AICC_REL_PARTS\" \"$EXPECTED_AICC_REL_FINGERPRINT\" \"$FINGERPRINTS\"\n" "l1b_b2_assert_triplet catalog AICC_COL \"$EXPECTED_AICC_COL_PARTS\" \"$EXPECTED_AICC_COL_FINGERPRINT\" \"$FINGERPRINTS\"\n" "l1b_b2_assert_triplet catalog AICC_POL \"$EXPECTED_AICC_POL_PARTS\" \"$EXPECTED_AICC_POL_FINGERPRINT\" \"$FINGERPRINTS\"\n" "l1b_b2_assert_triplet catalog AICC_FUN \"$EXPECTED_AICC_FUN_PARTS\" \"$EXPECTED_AICC_FUN_FINGERPRINT\" \"$FINGERPRINTS\"\n" "l1b_b2_assert_catalog L0B 234 \"$EXPECTED_L0B_FINGERPRINT\" \"$FINGERPRINTS\"\n" "l1b_b2_assert_catalog LINE 127 \"$EXPECTED_LINE_FINGERPRINT\" \"$FINGERPRINTS\"\n\n" "l1b_b2_set_stage 'index'\n\n")
