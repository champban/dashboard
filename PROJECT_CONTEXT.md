...
- The PostgreSQL 17 proof covers four paths: the trigger guard rejects a raced completing cycle; the RPC proof shows a same-owner call waits on the advisory lock while a third session can still lock its target task row with `NOWAIT`; the mixed direct UPDATE-versus-RPC proof forces the previous reverse lock order, requires immediate `L1D02`, proves the RPC owns the advisory lock while waiting on the tuple, rejects any `40P01`, and lets the RPC complete; and a shared-lock negative probe must fail `L1D02`. FIFO driver signals remove fixed-duration release races. The graph remains acyclic and distinct owners still complete independently. The dedicated workflow paths include every migration, contract, SQL test, operation and proof script consumed by the job.
