...
- The PostgreSQL 17 proof covers both paths: the trigger guard rejects a raced completing cycle, and the RPC proof shows a same-owner call waits on the advisory lock while a third session can still lock its target task row with `NOWAIT`. The RPC session-one transaction is held by a test-driver FIFO until the waiter, `NOWAIT`, and distinct-owner probes finish, removing the fixed-duration `pg_sleep` release race. Both proofs require `L1D01 dependency_cycle`, an acyclic final graph, and independent completion for a distinct owner.
