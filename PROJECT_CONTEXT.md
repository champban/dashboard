...
- The candidate enters the owner-scoped transaction advisory lock helper at the `task.children.replace` RPC before any per-task row lock. Direct INSERT uses the same helper from its trigger. A direct active-edge UPDATE must already carry the helper's transaction-local owner marker and the exact exclusive lock; otherwise it fails immediately with `L1D02 dependency_lock_required`. Shared locks and exclusive session locks without the marker are rejected, so a caller cannot release serialization before commit. UPDATE validation continues to use immutable `OLD` identity.
- The PostgreSQL 17 proof covers five paths: raced trigger-cycle rejection; owner-lock-before-row RPC ordering with a third-session `NOWAIT` probe; mixed direct UPDATE-versus-RPC reverse ordering with immediate `L1D02`, no `40P01`, and successful RPC; shared-lock rejection; and session-lock rejection followed by a positive helper-based UPDATE. FIFO driver signals remove fixed-duration release races. The graph remains acyclic and distinct owners still complete independently. The dedicated workflow paths include every migration, contract, SQL test, operation and proof script consumed by the job.
- L1A migration/contract are byte-identical at SHA-256 `46a721d90c1a66c4977c42d48958b45e6ca85dcfe678575174f7eac80c27fb30`, Git blob `036d010bcb79be939219415e977085ec55392d59`, 37,501 bytes. L1B migration/contract are byte-identical at SHA-256 `65fd6a7c4f1afdac85fd4367f1ff35ddc5ff6a00ff27097ab6b1dff660077713`, Git blob `38b1c9f719f66cdfea8b0a89d265888ed27a4a47`, 48,699 bytes, pinned by source provenance commit `c540d0b53dbb98607cf4f2f2ebb899e8d1480a7d` (parent `20e507faed12d872eae7549e79f9c6811a53f329`, `2026-08-29T18:39:42Z`). Historical ZIP evidence is superseded for both changed L1A/L1B bytes. PR #96 remains Draft/no-go pending final exact-head CI and independent review. Fresh B-1 artifact `9709317492` is valid through `2026-08-30T04:28:13Z`, but exact B-2 restore PASS is still absent; no Production action is authorized.
...
| L1-DEPENDENCY-LOCK-1 | Direct dependency UPDATE could reverse the RPC lock order; shared locks could bypass serialization; an exclusive session lock could be released before commit | A row-level UPDATE trigger runs after the tuple lock, mode-only `pg_locks` inspection does not reveal advisory-lock lifetime, and the first guard lacked transaction-owned provenance | Centralize acquisition in `private.mtp_l1_lock_dependency_graph`, which takes `pg_advisory_xact_lock` and records a transaction-local owner marker. RPC and INSERT use the helper; UPDATE requires marker + exact exclusive lock and fails `L1D02` otherwise | Byte parity; mixed UPDATE/RPC, shared-lock and session-lock negative proofs; helper positive proof; no `40P01`; exact-head CI and independent review |
