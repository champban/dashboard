...
- Owner separately approved PR #86 exact head
  `4830b6cf82aa1ff65306b775e2382d84e96af21e` against exact base
  `main@167b84cfdfeedd19c0396b2f520e9806244eec3b`. It merged as
  `main@8fc88a8a94017eadb58b98adecbb87e22d65496c`, tree
  `34f3859b997a530d80c4387bca0212388b731dc7`, which is byte-identical to the
  approved PR-head tree. Push verification run `32639538682` passed all four
  jobs and GitHub Pages run `32639537950` completed build/report/deploy
...
### L0b Production schema-only apply — catalog verified, import not executed
...
- LINE and unrelated `aicc_*` aggregate catalog/ACL/policy/function/count
  canaries were identical before and after apply. PR #86 later published the
  reviewed signed-in manual controls with `UI_ENABLED=true`; no timer,
  Auto-sync, Drive or LINE path invokes them. Browser + Google Drive remain
  authoritative and no import has run.
...
- PR #86 was approved only at exact head
  ...
  `34f3859b997a530d80c4387bca0212388b731dc7`, against exact base
  `main@167b84cfdfeedd19c0396b2f520e9806244eec3b`, tree
  `f53963dc82e088c0805cc8da94653352695c7865`. It merged without source drift as
  `main@8fc88a8a94017eadb58b98adecbb87e22d65496c`, with the same tree.
- The runtime change remains the reviewed `db3c8cded9359b402eb6316bb4c21067db8195d4`
  source commit: `l0b-import.js` changes `UI_ENABLED=false` to `true`, and
  `build/l0b-import.test.mjs` updates the matching assertions. The PR-head
  documentation commit changes no runtime bytes. Full/Mobile retain their
  `enabled===true` render checks and fail-closed handlers; no timer, Auto-sync,
  Drive or LINE path invokes `importNow`.
- Exact PR-head `verify` #151, run `32624687421`, and post-merge push `verify`
  #152, run `32639538682`, both passed all four jobs. GitHub Pages deployment
  #117, run `32639537950`, completed build, report and deploy successfully for
  exact merge commit `8fc88a8a` and published
  https://champban.github.io/dashboard/.
- Targeted 6D found no new Critical/High issue. The approved merge/publication
  boundary is closed; planner-data read/projection, first import, bounded
  reconciliation, acceptance, provider/Auth/secret change, cleanup and L1
  remain outside it.
- Post-publication read-only Supabase verification found Production
...
- The candidate acquires the owner-scoped transaction advisory lock at the `task.children.replace` RPC entry before its helper takes any per-task row lock. Direct INSERT retains the trigger guard. A direct active-edge UPDATE must already hold the same **exclusive** transaction lock before the statement; otherwise the row trigger fails immediately with `L1D02 dependency_lock_required` instead of waiting after a tuple lock and risking a mixed UPDATE-versus-RPC deadlock. A shared advisory lock is explicitly rejected. UPDATE validation continues to use immutable `OLD` identity.
- The PostgreSQL 17 proof covers four paths: the trigger guard rejects a raced completing cycle; the RPC proof shows a same-owner call waits on the advisory lock while a third session can still lock its target task row with `NOWAIT`; the mixed direct UPDATE-versus-RPC proof forces the previous reverse lock order, requires immediate `L1D02`, proves the RPC owns the advisory lock while waiting on the tuple, rejects any `40P01`, and lets the RPC complete; and a shared-lock negative probe must fail `L1D02`. FIFO driver signals remove fixed-duration release races. The graph remains acyclic and distinct owners still complete independently. The dedicated workflow paths include every source contract read by the proof, including `supabase/contracts/l1b_private_storage.sql`.
- L1A migration/contract are byte-identical at SHA-256 `b0cc480974995d15e667b14176e0ff70b77e34f977d83c732e1439a1a32b98fe`, Git blob `4cc4821bc023801ea3501400340c7ff57a28f71a`, 36,382 bytes. L1B migration/contract remain byte-identical at SHA-256 `9980557bd01830a36da3da35a7de6f3e418a4b0fb82db1431e6d736f74ee88d4`, Git blob `f763e5ff25da166e35d569c76c35022884c956cd`, 48,775 bytes, pinned by source provenance commit `31446f105d8753dfa7d8e1548955963daa85428b` (parent `c2918ecdacba35ab5b4d77a74944a2e1a4df6635`, `2026-08-29T18:19:34Z`). Historical ZIP evidence is superseded for both changed L1A/L1B bytes. PR #96 remains Draft/no-go pending final exact-head CI and independent review. Fresh B-1 artifact `9709317492` is valid through `2026-08-30T04:28:13Z`, but exact B-2 restore PASS is still absent; no Production action is authorized.
...
| L1-DEPENDENCY-LOCK-1 | Direct dependency UPDATE could hold a tuple and then wait for the owner advisory lock while `task.children.replace` held the advisory lock and waited for that tuple; a shared lock could also bypass serialization | A row-level UPDATE trigger runs only after PostgreSQL has locked the target tuple, and the first pre-lock predicate did not distinguish shared from exclusive mode | Direct active-edge UPDATE must pre-acquire the exact exclusive owner transaction lock; the trigger checks the current backend's lock mode and fails immediately with `L1D02` instead of waiting. RPC remains owner-lock-before-row; direct INSERT keeps its graph guard | Exact L1A contract/migration hash and byte parity; deterministic PostgreSQL 17 mixed UPDATE-versus-RPC proof plus shared-lock negative probe require `L1D02`, no `40P01`, and successful RPC; exact-head CI and independent review |
| L1-ARTIFACT-PATH-1 | Storage contract-only changes could skip the dedicated artifact parity/failure-safety workflow | The workflow path filter listed the Storage operation but omitted the source contract that the proof reads and compares | Include `supabase/contracts/l1b_private_storage.sql` in the dedicated workflow paths; keep exact contract/operation parity inside the proof | Workflow path contract review; exact-head dedicated proof and independent review |
