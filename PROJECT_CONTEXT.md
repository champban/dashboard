...
| LINE-4 | No runtime monitoring | **Built, not yet verified.** `.github/workflows/line-health.yml` runs `build/line-health-check.mjs` every 2 days: function GET → 405, unsigned POST → 401, anonymous snapshot read → denied (401/403). The third doubles as the keepalive for the free-tier 7-day pause and is the only automated guard on the `anon` revoke, so this replaces the separate keepalive cron in Infrastructure notes. Needs no repository secret — every endpoint is already public. **The live path has never run**: the development sandbox's egress proxy refuses CONNECT to `supabase.co`, and `workflow_dispatch` cannot be triggered until the workflow reaches `main`. `--selftest` (offline, in `npm test`) proves the logic both directions. Close this only after a real run passes. |
