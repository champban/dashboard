...
- The candidate acquires the owner-scoped transaction advisory lock at the `task.children.replace` RPC entry before its helper takes any per-task row lock. Direct INSERT retains the trigger guard. A direct active-edge UPDATE must already hold the same transaction lock before the statement; otherwise the row trigger fails immediately with `L1D02 dependency_lock_required` instead of waiting after a tuple lock and risking a mixed UPDATE-versus-RPC deadlock. UPDATE validation continues to use immutable `OLD` identity.
- The PostgreSQL 17 proof covers three paths: the trigger guard rejects a raced completing cycle; the RPC proof shows a same-owner call waits on the advisory lock while a third session can still lock its target task row with `NOWAIT`; and the mixed direct UPDATE-versus-RPC proof forces the previous reverse lock order, requires immediate `L1D02`, proves the RPC owns the advisory lock while waiting on the tuple, rejects any `40P01`, and lets the RPC complete. FIFO driver signals remove fixed-duration release races. The graph remains acyclic and distinct owners still complete independently.
- L1A migration/contract are byte-identical at SHA-256 `4bac012e5fa1375a89d207e669c60a52957bcddca512791952f0bc5580e9020a`, Git blob `5796a524a46d380aca0fb1fc941e497589f1e36e`, 36,344 bytes. L1B migration/contract remain byte-identical at SHA-256 `9980557bd01830a36da3da35a7de6f3e418a4b0fb82db1431e6d736f74ee88d4`, Git blob `f763e5ff25da166e35d569c76c35022884c956cd`, 48,775 bytes, pinned by source provenance commit `cef3d1667da1f67fa1f12c9ea02a3f94573ad173` (parent `8376b10fc59d62402dcef3f571dc889b1739beef`, `2026-08-29T18:06:16Z`). Historical ZIP evidence is superseded for both changed L1A/L1B bytes. PR #96 remains Draft/no-go pending final exact-head CI and independent review. Fresh B-1 artifact `9709317492` is valid through `2026-08-30T04:28:13Z`, but exact B-2 restore PASS is still absent; no Production action is authorized.
...
| L1-DEPENDENCY-LOCK-1 | Direct dependency UPDATE could hold a tuple and then wait for the owner advisory lock while `task.children.replace` held the advisory lock and waited for that tuple | A row-level UPDATE trigger runs only after PostgreSQL has locked the target tuple, so a blocking advisory lock inside that trigger reversed the RPC lock order | Direct active-edge UPDATE must pre-acquire the exact owner transaction lock; the trigger checks the current backend's advisory lock and fails immediately with `L1D02` instead of waiting. RPC remains owner-lock-before-row; direct INSERT keeps its graph guard | Exact L1A contract/migration hash and byte parity; deterministic PostgreSQL 17 mixed UPDATE-versus-RPC proof requires `L1D02`, no `40P01`, and successful RPC; exact-head CI and independent review |
